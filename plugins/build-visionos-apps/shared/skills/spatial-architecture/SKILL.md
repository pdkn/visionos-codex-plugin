...
- Switch to `swiftui-spatial` when the surface and ownership model
  are already chosen and the next step is implementing SwiftUI APIs.
- Switch to `realitykit` when the work is mainly about
  entities, components, systems, or RealityKit runtime behavior.
- Switch to `arkit` when the architecture choice depends on
  provider constraints, anchors, or tracked-world behavior.
- Switch to `shareplay` when the app structure is driven by group
...
- the next implementation handoff:
  `swiftui-spatial`, `realitykit`,
  `arkit`, `shareplay`, or `build-run-debug`
