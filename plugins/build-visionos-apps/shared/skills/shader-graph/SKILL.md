...
3. Route text-level USD structure edits to `usd`.
...
- Switch to `usd` when the task is really about prim paths,
  composition, or text-level USD authoring.
- Switch to `realitykit` when the blocker is entity setup,
...
- explicit routing to `usd` or RealityKit work if needed
